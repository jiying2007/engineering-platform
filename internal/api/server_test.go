...
func TestWorkTaskRunVerticalSlice(t *testing.T) {
	...
	taskBody := mustRequest(t, handler, http.MethodPost, "/api/v1/task-contracts", task, http.StatusCreated)
	var taskResponse struct {
		Digest string `json:"digest"`
	}
	if err := json.Unmarshal(taskBody, &taskResponse); err != nil {
		t.Fatal(err)
	}
	if taskResponse.Digest == "" {
		t.Fatal("expected immutable task contract digest")
	}
