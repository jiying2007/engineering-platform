...
func seedRestoreAuthority(t *testing.T, s *Store) {
	...
	if err := s.CreateWork(work); err != nil { t.Fatal(err) }
	...
	planDigest, err := plan.Digest(); if err != nil { t.Fatal(err) }
	...
	taskDigest, err := task.Digest(); if err != nil { t.Fatal(err) }
	ready := work; ready.ActiveTaskContractDigest = taskDigest; if err := ready.Transition(core.WorkReady); err != nil { t.Fatal(err) }
	if err := s.CreateTaskAndUpdateWork(task, plan, work.Version, ready); err != nil { t.Fatal(err) }
	...
	inputDigest, err := input.Digest(); if err != nil { t.Fatal(err) }
	value := run.New(input.RunID, taskDigest, inputDigest)
	attempt, err := value.StartAttempt("attempt", now.Add(time.Second)); if err != nil { t.Fatal(err) }
	sess := session.New(value.ID, attempt.Epoch)
	executing := ready; executing.ActiveRunID = value.ID; if err := executing.Transition(core.WorkExecuting); err != nil { t.Fatal(err) }
	if err := s.CreateExecutionAndUpdateWork(*value, attempt, *sess, input, ready.Version, executing); err != nil { t.Fatal(err) }
	currentRun, currentSession, _ := s.GetExecution(value.ID); currentWork, _ := s.GetWork(work.ID)
	if err := currentRun.Complete(currentRun.CurrentEpoch); err != nil { t.Fatal(err) }
	verifying := currentWork; if err := verifying.Transition(core.WorkVerifying); err != nil { t.Fatal(err) }
	if err := s.UpdateExecutionAndWork(value.ID, currentRun.Version, currentRun, currentSession, currentWork.Version, verifying); err != nil { t.Fatal(err) }
	...
	delivery.SubjectDigest, err = delivery.CalculateSubjectDigest(); if err != nil { t.Fatal(err) }
	if err := s.CreateDelivery(delivery); err != nil { t.Fatal(err) }
	...
	if err := s.CreateEvidence(evidence); err != nil { t.Fatal(err) }
	verificationReport := verification.Evaluate(plan, delivery.SubjectDigest, []core.EvidenceRef{evidence})
	verificationReport.ID = "restore-verification"; verificationReport.DeliveryReceiptID = delivery.ID; verificationReport.VerificationPlanDigest = planDigest; verificationReport.Verifier = "verifier"; verificationReport.CreatedAt = now.Add(3*time.Second)
	if err := s.CreateVerification(verificationReport); err != nil { t.Fatal(err) }
	...
	reviewing := verifying; if err := reviewing.Transition(core.WorkReviewing); err != nil { t.Fatal(err) }
	if err := s.CreateReviewAndUpdateWork(reviewReport, verifying.Version, reviewing); err != nil { t.Fatal(err) }
	...
	closed := reviewing; if err := closed.Transition(core.WorkClosed); err != nil { t.Fatal(err) }
	if err := s.CreateClosureAndUpdateWork(closure, reviewing.Version, closed); err != nil { t.Fatal(err) }
	...
	if err := s.Create(*op); err != nil { t.Fatal(err) }
	if err := op.Transition(action.Dispatched, now.Add(7*time.Second)); err != nil { t.Fatal(err) }; if err := s.Update(*op); err != nil { t.Fatal(err) }
	if err := op.Transition(action.Confirmed, now.Add(8*time.Second)); err != nil { t.Fatal(err) }; if err := s.Update(*op); err != nil { t.Fatal(err) }
	if _, err := s.BeginRecovery(0); err != nil { t.Fatal(err) }
	if _, err := s.CreateRecoveryProof(context.Background(), 1, "urn:engineering-platform:operator:reconciler"); err != nil { t.Fatal(err) }
}

func authoritySnapshot(ctx context.Context, s *Store) (restoreSnapshot, error) {
	var snapshot restoreSnapshot
	var err error
	if snapshot.Recovery, err = s.GetRecovery(); err != nil { return snapshot, err }
	work, err := s.GetWork("restore-work"); if err != nil { return snapshot, err }; snapshot.WorkDigest, _ = canonical.Digest(work)
	delivery, err := s.GetDelivery("restore-delivery"); if err != nil { return snapshot, err }; snapshot.DeliveryDigest, _ = canonical.Digest(delivery)
	evidence, err := s.GetEvidence("restore-evidence"); if err != nil { return snapshot, err }; snapshot.EvidenceDigest, _ = canonical.Digest(evidence)
	verificationReport, err := s.GetVerification("restore-verification"); if err != nil { return snapshot, err }; snapshot.VerificationDigest, _ = canonical.Digest(verificationReport)
	reviewReport, err := s.GetReview("restore-review"); if err != nil { return snapshot, err }; snapshot.ReviewDigest, _ = reviewReport.Digest()
	closure, err := s.GetClosure("restore-closure"); if err != nil { return snapshot, err }; snapshot.ClosureDigest, _ = canonical.Digest(closure)
	op, err := s.Get("restore-operation"); if err != nil { return snapshot, err }; snapshot.OperationDigest, _ = canonical.Digest(op)
	proof, err := s.GetRecoveryProof(ctx, 1); if err != nil { return snapshot, err }; snapshot.ProofDigest, _ = proof.Digest()
	if err := s.pool.QueryRow(ctx, "SELECT last_sequence,last_digest FROM audit_journal_state WHERE singleton_id=true").Scan(&snapshot.AuditSequence, &snapshot.AuditDigest); err != nil { return snapshot, err }
	rows, err := s.pool.Query(ctx, "SELECT version FROM core_schema_migrations ORDER BY version"); if err != nil { return snapshot, err }
	for rows.Next() { var version uint64; if err := rows.Scan(&version); err != nil { rows.Close(); return snapshot, err }; snapshot.Migrations = append(snapshot.Migrations, version) }; rows.Close()
	rows, err = s.pool.Query(ctx, "SELECT state,count(*) FROM outbox_events GROUP BY state ORDER BY state"); if err != nil { return snapshot, err }
	for rows.Next() { var item restoreStateCount; if err := rows.Scan(&item.State, &item.Count); err != nil { rows.Close(); return snapshot, err }; snapshot.Outbox = append(snapshot.Outbox, item) }; rows.Close()
	return snapshot, rows.Err()
}

func restoreDatabaseName(t *testing.T, suffix string) string {
	t.Helper(); var nonce [8]byte; if _, err := rand.Read(nonce[:]); err != nil { t.Fatal(err) }; return "ep_restore_"+suffix+"_"+hex.EncodeToString(nonce[:])
}

func databaseURLWithName(t *testing.T, raw, name string) string {
	t.Helper(); parsed, err := url.Parse(raw); if err != nil { t.Fatal(err) }; parsed.Path = "/" + name; return parsed.String()
}

func runPG17(t *testing.T, raw string, args []string, mount string) {
	t.Helper(); parsed, err := url.Parse(raw); if err != nil { t.Fatal(err) }
	host := parsed.Hostname(); if host == "" { host = "127.0.0.1" }; port := parsed.Port(); if port == "" { port = "5432" }
	user := parsed.User.Username(); password, _ := parsed.User.Password()
	command := []string{"run", "--rm", "--network", "host", "-e", "PGPASSWORD="+password, "-v", mount+":/dump", "postgres:17-alpine"}
	command = append(command, args[0], "-h", host, "-p", port, "-U", user); command = append(command, args[1:]...)
	cmd := exec.Command("docker", command...); output, err := cmd.CombinedOutput(); if err != nil { t.Fatalf("PostgreSQL 17 %s failed: %v\n%s", args[0], err, output) }
}
