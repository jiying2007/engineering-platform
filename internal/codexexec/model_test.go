...
func TestProfileAndAssignmentAreExact(t *testing.T) {
	...
	if err := CheckAssignment(permit.Assignment, p); err != nil {
		t.Fatal(err)
	}
	bad := permit.Assignment
	bad.Input.ToolProfile = "codex/other"
	if err := CheckAssignment(bad, p); err == nil {
		t.Fatal("wrong tool profile accepted")
	}
	p.ToolNetwork = true
	if err := p.Validate(); err == nil {
		t.Fatal("network-enabled profile accepted")
	}
}

func TestPromptIdentityIsLocatorIndependent(t *testing.T) {
	_, permit := contractFixture(t)
	one, d1, err := Prompt(permit.Assignment, permit.Preparation, "/one/bundle")
	if err != nil {
		t.Fatal(err)
	}
	two, d2, err := Prompt(permit.Assignment, permit.Preparation, "/two/bundle")
	if err != nil {
		t.Fatal(err)
	}
	if d1 != d2 || one == two {
		t.Fatalf("prompt identity incorrectly includes locator: %s %s", d1, d2)
	}
	expected, err := PromptIdentityDigest(permit.Assignment, permit.Preparation)
	if err != nil || expected != d1 {
...
func TestResultValidationBindsPromptModelAndChange(t *testing.T) {
	...
	codex := codexapp.EngineeringReceipt{
		...
		Output:                     "done",
		OutputDigest:               canonical.BytesDigest([]byte("done")),
		CommandCount:               1,
		FileChangeCount:            1,
	...
	change := workspace.ChangeFacts{
		...
		BaseTree:           permit.Preparation.Facts.TreeCommit,
		BaseSourceDigest:   permit.Preparation.Facts.SourceDigest,
		ResultCommit:       strings.Repeat("8", 40),
		ResultTree:         strings.Repeat("9", 40),
		ResultSourceDigest: "sha256:" + strings.Repeat("b", 64),
	...
	if err := r.Validate(p, permit); err == nil {
		t.Fatal("forged prompt identity accepted")
	}
}
