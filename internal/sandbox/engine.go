...
func New(socket, guard string) (*Engine, error) {
	...
	}, MaxResponseHeaderBytes: 32 << 10, DisableCompression: true, ResponseHeaderTimeout: 15 * time.Second}
	return &Engine{guard: guard, guardDigest: Hash(data), transport: t, client: &http.Client{Transport: t, Timeout: 20 * time.Second, CheckRedirect: func(*http.Request, []*http.Request) error { return ErrPolicy }}}, nil
}
