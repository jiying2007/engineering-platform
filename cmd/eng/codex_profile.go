...
func codexProfile(args []string) error {
	...
	if err := fs.Parse(args); err != nil || fs.NArg() != 0 || strings.TrimSpace(*executable) == "" || strings.TrimSpace(*model) == "" {
...
func buildCodexProfile(executable, model string) (codexProfileOutput, error) {
	...
	if got := strings.TrimSpace(string(stdout.data)); got != "codex-cli "+codexapp.QualifiedCodexVersion {
		return output, fmt.Errorf("Codex version mismatch: got %q want %q", got, "codex-cli "+codexapp.QualifiedCodexVersion)
	}
...
type boundedProfileOutput struct{ data []byte }
